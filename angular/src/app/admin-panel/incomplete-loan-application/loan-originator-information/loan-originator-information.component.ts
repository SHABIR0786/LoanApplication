...
export class LoanOriginatorInformationComponent implements OnInit {
  ...
  constructor(private loanOriginatorInfoService: LoanOriginatorInfoService, private borrowService: BorrowService, private declarationsService: DeclarationsService, private militaryServicesService: MilitaryServicesService, private loanPropertyInfoService: LoanPropertyInfoService, private financialInfoRealEstateService: FinancialInfoRealEstateService, private financialInfoService: FinancialInfoAssetsLiabilitiesService, private demographicInfoService: DemographicInfoService, private router: Router) { }

  ngOnInit(): void {
    if (localStorage.loanOriginatorInfoModels != undefined && localStorage.loanOriginatorInfoModels != '') {
      this.loanOriginatorInfoModels = JSON.parse(localStorage.getItem('loanOriginatorInfoModels'));
    }
  }
  create() {
