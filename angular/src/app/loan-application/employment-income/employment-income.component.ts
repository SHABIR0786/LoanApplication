...
export class EmploymentIncomeComponent implements OnInit, DoCheck {
    ...
    initForm() {
        this.form = new FormGroup({
            ...
            borrowerMonthlyIncome: this.initBorrowerMonthlyIncome(this.data.borrowerMonthlyIncome || {borrowerTypeId: 1}),
            borrowerEmploymentInfo: new FormArray([]),
            additionalIncomes: new FormArray([]),
        });

        if (this.data.borrowerEmploymentInfo && this.data.borrowerEmploymentInfo.length) {
            this.data.borrowerEmploymentInfo.forEach(d => this.addBorrowerEmploymentInfo(d));
        } else {
        ...
        if (this.isApplyingWithCoBorrower) {
            ...
            this.form.addControl('coBorrowerMonthlyIncome', this.initBorrowerMonthlyIncome(this.data.coBorrowerMonthlyIncome || {borrowerTypeId: 2}));

            if (this.data.coBorrowerEmploymentInfo && this.data.coBorrowerEmploymentInfo.length) {
                this.data.coBorrowerEmploymentInfo.forEach(d => this.addBorrowerEmploymentInfo(d));
            } else {
    ...
    addBorrowerEmploymentInfo(data?: IBorrowerEmploymentInfoModel) {
        this.borrowerEmploymentInfo.push(this.newBorrowerEmploymentInfo(data || {borrowerTypeId: 1}));
    }

    addCoBorrowerEmploymentInfo(data?: IBorrowerEmploymentInfoModel) {
        this.coBorrowerEmploymentInfo.push(this.newBorrowerEmploymentInfo(data || {borrowerTypeId: 2}));
    }

    addAdditionalIncome(data?: IAdditionalIncomeModel) {
        this.additionalIncomes.push(this.newAdditionalIncome(data || {borrowerTypeId: 1}));
    }
