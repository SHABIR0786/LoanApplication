    ...
    initForm() {
        ...
        this.form
            ...
            if (downPaymentAmount) {
                const estimatedPurchasePrice = this.form.get('estimatedPurchasePrice').value;
        ...
        if (event === 'wizardStep') {
            ...
            switch (stepIndex) {
                case 1:
                    ...
                    const hasError = fields.some(field => this.form.get(field) && !this.form.get(field).valid);

                    if (hasError) {
                        fields.forEach(field => this.form.get(field) && this.form.get(field).markAsTouched());
                    } else {
